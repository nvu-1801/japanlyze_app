...
class _ConversationLessonPageState extends State<ConversationLessonPage> {
  ...
  void _toggleItemComplete(int index) {
    setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        centerTitle: false,
        bottom: _progress > 0 && _progress < 1.0
            ? PreferredSize(
      ...
      body: widget.lesson.vocabItems.isNotEmpty
  ...
  Widget _buildVocabList(BuildContext context) {
    return ListView.builder(
      ...
      itemCount: widget.lesson.vocabItems.length + 1,
      itemBuilder: (context, index) {
        if (index == widget.lesson.vocabItems.length) {
          return _buildCompletionButton(context);
        }
        return _VocabCard(
          item: widget.lesson.vocabItems[index],
  ...
  Widget _buildDialogueList(BuildContext context) {
    return ListView.builder(
      padding: const EdgeInsets.fromLTRB(16, 8, 16, 100),
      itemCount: widget.lesson.lines.length + 1,
      itemBuilder: (context, index) {
        if (index == widget.lesson.lines.length) {
          return _buildCompletionButton(context);
        }
        return _DialogueCard(
          line: widget.lesson.lines[index],
          isCompleted: _completedItems.contains(index),
          onTap: () => _toggleItemComplete(index),
        );
      },
...
class _DialogueCardState extends State<_DialogueCard> {
  ...
  void initState() {
    ...
    if (widget.isCompleted) {
      _answered = true;
      _selectedIndex = widget.line.correctOptionIndex;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: widget.isCompleted ? widget.onTap : null,
      child: Container(
